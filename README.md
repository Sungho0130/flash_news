...
- 저희가 사용한 'Bertshared-kor-base' 모델은 Bert가 기존에 가진 생성 및 요약에 취약점을 보완하기 위한 모델이다.
- BERTShared는 seq2seq 방식을 착안하여 Bert모델 하나로 파라미터를 공유하여 decoder가 약한 bert모델로도 생성 및 요약이 가능한 모델이다.
- 너무 짧은 문장, 중복되는 문장들을 제외하여 100GB의 데이터 중 최종적으로 70GB (약 127억개의 token)의 텍스트 데이터가 학습된 모델이며, 이외에도 화장품(8GB), 식품(6GB), 전자제품(13GB), 반려동물(2GB) 카테고리별로 학습되었다.
- Dataset
