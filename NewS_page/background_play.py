...
def data_num_ct(category):
    ...
    if current_records > max_records:
        # 최신 데이터 기준으로 오래된 레코드를 선택
        records_to_delete = Crawring_ct.objects.filter(category=category).order_by('created_at')[:current_records - max_records]
...
def main():
    ...
    sched.add_job(job,'interval', minutes=30, id='test', next_run_time=datetime.now())
    sched.start()
